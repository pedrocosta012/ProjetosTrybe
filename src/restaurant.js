...
const createMenu = (menu) => { 
  const menuObject = {
    fetchMenu: () => menu, 
    consumption: [], 
    order: (newFood) => menuObject.consumption.push(newFood),
    pay: () => {
      let totalPayment = 0;
      for (let i = 0; i < menuObject.consumption.length; i += 1) {
        if (Object.keys(menuObject.fetchMenu().food).includes(menuObject.consumption[i])) {
          totalPayment += menuObject.fetchMenu().food[menuObject.consumption[i]];
        } else if (Object.keys(menuObject.fetchMenu().drink).includes(menuObject.consumption[i])) {
          totalPayment += menuObject.fetchMenu().drink[menuObject.consumption[i]];
        }
      }
      return totalPayment;
    } };
