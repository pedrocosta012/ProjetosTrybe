...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    const menuObject = createMenu(objetoQualquer);
    expect(Object.keys(menuObject.fetchMenu())[0]).toContain('food');
    expect(Object.keys(menuObject.fetchMenu())[1]).toContain('drink');
    ...
    // ```
    expect(menuObject.fetchMenu()).toEqual(objetoQualquer);
    ...
    // ```
    menuObject.order('Pastel de Frango')
    expect(menuObject.consumption.includes('Pastel de Frango')).toEqual(true);
    ...
    // ```
    const adicionaItens = createMenu(objetoQualquer);
    adicionaItens.order('coxinha');
    adicionaItens.order('agua');
    adicionaItens.order('sopa');
    adicionaItens.order('sashimi');
    expect(adicionaItens.consumption).toEqual(['coxinha', 'agua', 'sopa', 'sashimi']);
    ...
    // ```
    const adicionaRepetidos = createMenu(objetoQualquer);
    adicionaRepetidos.order('coxinha');
    adicionaRepetidos.order('agua');
    adicionaRepetidos.order('coxinha');
    expect(adicionaRepetidos.consumption).toEqual(['coxinha', 'agua', 'coxinha']);
    ...
    // ```
    const calcPagamento = createMenu(objetoQualquer);
    calcPagamento.order('coxinha');
    calcPagamento.order('agua');
    calcPagamento.order('coxinha');
    expect(calcPagamento.pay()).toBe(11.7);
